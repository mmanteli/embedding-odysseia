...
import sys
from embedding_extraction.extract import transform
from embedding_extraction.faissify import make_training_sample, train_faiss, index_w_fais
from embedding_extraction.sanity_check import get_NN
import os
import glob
import pathlib
...
parser.add_argument('--data','--embedded', '--save', type=str,
                    help="Path to save embedings in .pkl format, can only be dir, then set data_suffix.", default=None)
parser.add_argument('--data_suffix', type=str, default=None, help="See --data.")
parser.add_argument('--batch_size', '--batchsize', type=int,
                    help="Document batch size for embeddings", default = 500)
parser.add_argument('--model_batch_size', type=int,
                    help="Model inner batch size", default=32)  # tested to be the fastest out of 32 64 128
parser.add_argument('--split_by', default="sentences", choices=["tokens", "sentences", "chars", "words", "truncate"],
                    help='What to use for splitting too long texts, truncate=nothing')
parser.add_argument('--chunk_size', type=int,
                    help="How many units (tokens, words, chars) per batch. For sentences, splits up by char, \
                        for tokens, automatically set to model_max_len -2", default = None)
parser.add_argument('--overlap', '--context_overlap', type=int,
                    help="How much overlap per segment", default = None)
parser.add_argument('--temporary_training_set', '--temp', type=str,
                    help='sample training data to temporary .pkl, read to .pt in indexing')
...
if options.embed:
    ...
    assert options.data_suffix is not None or ".pkl" in options.data, (
        f"Give the saving location for embeddings as dir (--data={options.data}) and \
             suffix (--data_suffix={options.data_suffix}), or --data as one .pkl file"
    )
    # check format
...
if options.index:
    assert (
        options.base_indexer is not None and options.training_data is not None
    ) or options.trained_indexer is not None, "Give either base indexer and training data or trained indexer to index."
...
if options.sanity_check:
    assert options.filled_indexer is not None and options.database is not None, (
        "Give filled indexer and databse to do the sanity check"
    )
...
if options.embed:
    ...
    if options.temporary_training_set:
        ...
        assert (
            save_embed_file[-4:] == ".pkl"
            and temp_train_file[-4:] == ".pkl"
            and "," not in save_embed_file
            and "," not in temp_train_file
        ), f"--data or --temporary_training_set read incorrectly save = {save_embed_file}, temp = {temp_train_file} "
        ...
        if options.debug:
            print(
                f"Running embedding calculations, saving to {save_embed_file}, \
                     and creating temp training set to {temp_train_file}",
                flush=True,
            )
    ...
    # If no training data created in this step
        ...
        save_embed_file = os.path.join(options.data, options.data_suffix) if options.data_suffix else options.data
        # assert that extension is correct and that list is only given with a shard
        assert save_embed_file[-4:] == ".pkl" and "," not in save_embed_file, (
            f"--data or --temporary_training_set read incorrectly save = {save_embed_file}, temp = {temp_train_file} "
        )
        # make directories if needed
        if os.path.dirname(save_embed_file):
        ...
        if options.debug:
            print(
                f"Running embedding calculations, saving to {save_embed_file}, not creating temp training set.",
                flush=True,
            )
...
if options.index:
    ...
    indexer = None  # this is so we can differentiate if training was done in this step or earlier,
    # we do not have to load the index if we just trained it
    ...
    if not pathlib.Path(options.trained_indexer).is_file():
        ...
        if not pathlib.Path(options.training_data).is_file():
            if options.debug:
                print("Did not find training data in pytorch format")
            if options.temporary_training_set:
                if options.debug:
                    print(f"Creating from temp training data {options.temporary_training_set}")
                training_sample_exists = make_training_sample(
                    options.temporary_training_set,
                    options.training_data,
                    debug=options.debug,
                    fraction=training_fraction_for_sampled,
                )  # NOTE full data here
            else:
                training_sample_exists = make_training_sample(
                    options.data,
                    options.training_data,
                    debug=options.debug,
                    fraction=options.threshold,
                )
            assert training_sample_exists, (
                "Could not make training data for reasons that should have been reported above"
            )
        if options.debug:
            print("Now moving to training faiss index", flush=True)
        indexer = train_faiss(options)
    if options.debug:
        print("Now moving to indexing", flush=True)
    index_w_fais(options, index=indexer)

### Sanity
if options.sanity_check:
    if options.debug:
        print("In sanity check", flush=True)
    get_NN(options)
