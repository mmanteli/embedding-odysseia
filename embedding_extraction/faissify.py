...
def yield_from_pickle(f_names):
    """
    Generate iteratee from a list of pickled files (or one pickle file).
    """
    ...
    for f_name in f_names:
        with open(f_name,"rb") as f:
            while True:
                try:
                    dicts=pickle.load(f)
                    if type(dicts) is list:   # some older sets are saved this way
                        for d in dicts:
                            yield d
                    else:
                        yield dicts
...
def make_training_sample(data, training_data, debug=False, fraction = 0.1):
    ...
    """
    if debug:
        print(f'Making a training set from {data} with fraction {fraction}, saving to {training_data}', flush=True)
    training_embeddings = []
    try:
...
def train_faiss(options):
    ...
    if options.base_indexer == "IVFPQ":
        ...
        # using only FlatL2
        if options.debug:
            print("Using FlatL2 as the full indexer.", flush=True)
        index = quantizer
    if not index.is_trained:
        ...
        return index
    if options.debug:
        print("This index does not require training.", flush=True)
    return index


def index_w_fais(options, index=None):
    """Use trained faiss index to index, save filled index separately."""
    db = SqliteDict(options.database)
    if index is None:
        if options.debug:
            print(f"Reading {options.trained_indexer}...", flush=True)
    ...
    if emb_to_index != []:
        ...
        db.commit()
    if options.debug:
        print("Filling done, saving filled index", flush=True)
    index_filled = index
    faiss.write_index(index_filled, options.filled_indexer)
    if options.debug:
        print(f'{index_filled.ntotal} vectors in index.', flush=True)
    return True


def run(options):
    """
    Run indexing by checking if each step of the indexing pipeline is done.
    """
    # check if the training data is already done
    if not pathlib.Path(options.training_data).is_file():
        if options.debug:
            print("Did not find training data, making it from options.data.")
    ...
    if not pathlib.Path(options.trained_indexer).is_file():
        if options.debug:
            print("Did not find a trained indexer, training...")
