...
def find_path(index, db, start_query, target_query, target_neighbor_faiss_indices, options):
    """Find curvy nn-path between start and target."""
    path_indices = []
    path_distances = []
    path_points = []
    ...
    while max_iterations > 0:
        if options.debug:
            print("\n-------------------------\nNew iteration\n")
        closest_indices, distances, closest_points = get_NN(
            index,
            db,
            current_query,
            target_query,
        ...
        if any(match_target):  # we found one close to target
            if options.debug:
                print("Found nearest to target")
            # TODO ADD VALUES
            return True, path_indices, path_distances, path_points
        if all(match_path):  # we only found things we have visited before
            if options.debug:
                print("Back tracking")
...
def pathing(options, start_text=None, target_text=None):
    ...
    # finding the path
    success, path_indices, path_distances, path_points = find_path(
        index,
        db,
        start_query,
        target_query,
        target_neighbor_faiss_indices,
        options,
    )
